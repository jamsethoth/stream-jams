...
The repository SHALL configure Dependabot version updates for workspace package dependencies and GitHub Actions.

#### Scenario: Package update PR is eligible
- **WHEN** a compatible pnpm/npm dependency update is available
- **THEN** Dependabot is configured to open a pull request according to the repository schedule and grouping rules

#### Scenario: GitHub Actions update PR is eligible
- **WHEN** a GitHub Actions dependency update is available
- **THEN** Dependabot is configured to open a pull request for the workflow dependency
