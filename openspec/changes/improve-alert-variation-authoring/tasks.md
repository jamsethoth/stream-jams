...
- [x] 3.3 Implement core `validateAuthoredAlertConditions` catalog validation and enforce it in `AlertEditorService.saveDocument` by comparing candidate rule and variation conditions with their current saved conditions; allow unchanged unsupported conditions to round-trip or be removed, reject new, modified, or duplicated unsupported conditions, and complete this validation before any persistence mutation or transaction.
