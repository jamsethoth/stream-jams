...
Alert duration defaults to **Match longest media** for newly created content. It follows the longest visible local audio or video layer, falls back to 5 seconds when no readable duration is available, and caps playback at 2 minutes. Choose **Custom** to enter a fixed duration. Existing alerts remain Custom until changed. Audio layers and enabled video soundtracks each have independent **Fade in** and **Fade out** controls; enabling either starts at 500 milliseconds and exposes its exact duration. Preview, Browser Source and named-device playback use the same linear fade timing. TTS is not faded.
