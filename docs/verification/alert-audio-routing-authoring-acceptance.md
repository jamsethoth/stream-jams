...
| `pnpm test:unit` (unchanged default aggregate command) | 1,521 tests / 180 files passed in 222.80 seconds, exit 0 |
...
Commands above used `corepack.cmd pnpm` on Windows. The initial sandbox invocation could not access the installed Corepack cache; it was rerun with authorized runtime access. Two initial aggregate unit invocations (default pool and thread pool) were interrupted after producing no visible progress beyond the run banner; neither is counted as passed or evidence of a diagnosed root cause. The narrower project runs completed without the earlier worker-termination warnings and cover every configured unit-test file. The final unchanged default aggregate run subsequently completed all 1,521 tests with exit 0 and no worker-termination warnings, closing the aggregate verification gap; the earlier lack of visible progress does not establish a collection hang. The historical failed shutdown observations and BL-044 remain retained.
...
Both completed changes were synchronized into seven durable capability specs and moved to `openspec/changes/archive/2026-09-07-*`. Both additions to backup/restore remain present. Post-archive `openspec.cmd validate --all --strict --json` passed all 32 active items (30 specs and two unrelated proposals); archive/plan relative links and Git whitespace checks passed. Completed backlog entries BL-042/BL-043 were removed under the backlog maintenance rule; BL-030 distribution work and BL-044 remain deferred. Implementation and archive cleanup use separate commits. The unrelated changelog/video-shoutout proposals were not changed, and merge remains outside the requested publication scope.

### CI hardening

The [first PR validation run](https://github.com/jamsethoth/stream-jams/actions/runs/34148426451) passed build, Storybook, browser E2E, Windows desktop and CodeQL, but failed typecheck and dependency review. Those failures are not counted as passing:

- Dependency review identified `tar@6.2.1` through Forge's rebuild/node-gyp/cacache chain. The version-scoped override now resolves that chain to `tar@7.5.21`, including the fixes identified by the [critical decompression advisory](https://github.com/advisories/GHSA-23hp-3jrh-7fpw) and [recursive member-filter advisory](https://github.com/advisories/GHSA-r292-9mhp-454m). No security gate or advisory was bypassed. The upstream [file and streaming extraction API](https://github.com/isaacs/node-tar/tree/v7.5.21#high-level-api) remains applicable; both actual dependency paths passed a tiny synthetic archive extraction check. Its temporary fixture was removed, not any user QA profile.
- The local dependency layout had not resolved Electron's declaration through Playwright; a TypeScript resolution trace reported the module unresolved despite the preceding local checks. A normal install established the declared dependency tree and a forced check reproduced all three CI errors. The two desktop test files now give the existing runtime-only preferences inspector a narrow test-only type and register lifecycle listeners with literal event names accepted by Electron's overloads. Runtime assertions, application code, timeouts and native cleanup behavior are unchanged.

After correction, a real `pnpm install --frozen-lockfile --offline`, `pnpm exec tsc -b tsconfig.json --force`, lint, production build and Windows packaging passed. The rebuilt silent desktop gate passed **17 tests with 1 intentionally audible-gated skip in 31.7 seconds**, including the changed test files and real Settings/transport workflow. No Stream Jams process remained. Git whitespace checks passed. These are the focused post-correction results; the preceding full unit/Storybook/browser results remain historical, and the updated PR CI is authoritative for the final published head.

The [next CI run](https://github.com/jamsethoth/stream-jams/actions/runs/34154099080) passed every functional job but dependency review identified the separate [unpatched `extract-zip@2.0.1` symlink-traversal advisory](https://github.com/advisories/GHSA-jmr9-qjv8-65gv). Forge's Packager 18 is its only consumer. A scoped override backports the [official Electron extractor replacement](https://github.com/electron/packager/releases/tag/v20.0.1) to that dependency edge using `@electron-internal/extract-zip@1.0.5`, already present through Electron 44.1.1. The obsolete `yauzl` override and unused ZIP dependencies are removed. Packager itself stays on 18.4.4 because the [19.x hook signature changes](https://github.com/electron/packager/releases/tag/v19.0.0) are incompatible with Forge 7's callback hooks.

This is a build-time-only replacement for Electron distribution ZIPs fetched through `@electron/get`; it does not introduce a general-purpose extractor for user content. That use matches the replacement's [security scope](https://github.com/electron/extract-zip/blob/main/SECURITY.md). Packager passes only `{ dir }`, which matches the replacement API, and Node 24 supports its ESM default export through the existing CommonJS wrapper. The new regression resolves through the actual Forge/Packager dependency chain: before the override its identity assertion failed on the vulnerable package; after the override both tests passed, including exact extracted fixture contents and corrupt-archive rejection. Forced full typecheck, lint and a real frozen offline install also passed. No security exception was added.

The production build and actual Forge Windows package then passed using that replacement. The rebuilt silent desktop suite passed **17 tests with 1 intentionally audible-gated skip in 32.9 seconds**, retaining the existing assertions and native deadlines. The package archive was built at 19:15:12 UTC on September 7, and no Stream Jams process remained after the suite. The final published-head CI must pass dependency review as well as the unchanged functional gates before the PR is marked ready.
