...
| Hi-Fi - Alert Editor Landscape | `AlertEditorPage` provides focused routing, alert search/tree, canvas, layer controls, profile geometry, preview, save, and revert. | `AlertEditorPage.test.tsx`; `editor-state.test.ts`; `ReadyLandscape` story; `management-alerts.spec.ts`. | Preset animation is MVP; timeline/keyframe editing is backlog. |
...
| Hi-Fi - Alert Editor Send Test Blocked | Send-test failures remain visible with cause, next step, and reference context; unreviewed profiles disable dispatch. | `DeliveryFailure` story; send-test assertions in `AlertEditorPage.test.tsx`; server editor-service tests. | Full provider-event simulation across all enabled flows is backlog. |
...
| Hi-Fi - TTS Provider Setup | TTS uses its own capability-specific validation-before-registration wizard. | Provider registration tests; provider setup Storybook states. | Setup drafts and multiple active TTS providers are backlog. |
...
| Hi-Fi - Settings Overview | `SettingsPanel` owns theme, local server, data/log/version information, and backup/restore. | `SettingsPanel.test.tsx`; `Overview` story; settings Playwright flow. | Moving data, configurable retention, updates, and command palette are backlog. |
