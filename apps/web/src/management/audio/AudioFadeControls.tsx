import "./audio-fade-controls.css";
...
export function AudioFadeControls({ fadeInMs = 0, fadeOutMs = 0, onChange }: {
  ...
  return <fieldset className="audio-fade-controls">
    <legend>Audio fades</legend>
    <p className="audio-fade-controls__help">Fade volume smoothly at the beginning or end of this source.</p>
...
function FadeField({ label, value, onChange }: { readonly label: string; readonly value: number; readonly onChange: (value: number) => void }) {
  ...
  return <div className="audio-fade-controls__field">
    <label className="media-control-toggle"><input checked={enabled} onChange={(event) => onChange(event.currentTarget.checked ? 500 : 0)} type="checkbox" />{label}</label>
    {enabled ? <label className="audio-fade-controls__duration">{label} duration (milliseconds)<input aria-label={`${label} duration (milliseconds)`} max={120_000} min={1} onChange={(event) => {
