...
export function AssetPicker(props: AssetPickerProps) {
  ...
  useEffect(() => {
    ...
    void props.managementApi.listAssetLibraryItems().then((loaded) => {
      if (!active) return;
      setItems(loaded);
      setSelectedId((current) => current ?? loaded.find((item) => props.compatibleMediaTypes.includes(item.mediaType))?.id ?? null);
      setError(null);
    ...
    return () => { active = false; };
  }, [props.compatibleMediaTypes, props.managementApi, props.open]);
