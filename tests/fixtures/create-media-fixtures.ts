...
export async function recordNeutralClip(page: Page, options: { width: number; height: number; durationMs: number; withAudio: boolean; mimeType?: string }): Promise<Uint8Array> {
  ...
  const bytes = await page.evaluate(async ({ width, height, durationMs, withAudio, mimeType: requestedMimeType }) => {
    ...
    try {
      ...
      draw(); recorder.start();
      timer = setTimeout(() => { if (activeRecorder.state !== "inactive") activeRecorder.stop(); }, durationMs);
      await stopped;
